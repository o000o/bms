...
exports.soapCreateClient = () => {
    return new Promise((resolve, reject) => {
        ...
        soapNtlm.createClient(cfg.om.wsdlPath, cfg.om.options, (err, client, body) => {
            if (err) {
                reject({
                        code: error.code_03003,
                        desc: err
                    }) // failure
            } else {
                client.setEndpoint(cfg.om.endpoint)
                client.setSecurity(new soapNtlm.NtlmSecurity(cfg.om.options.wsdl_options))
                resolve(client)
            }
        })
    })
...
exports.createSoapClient = () => {
    return new Promise((resolve, reject) => {
        /*soapObj = {
          wsdl: __dirname+'/om.wsdl',
          wsdlOptions: {}
          endpoint: 'https://test-omws.ais.co.th/omws/WS_OM_OMService.svc',
          authenType: '0', // [0,null=No authen, 1=Basic]
          username: 'test',
          password: 'password'
        }*/
        process.env.NODE_TLS_REJECT_UNAUTHORIZED = 0
        soap.createClient(cfg.soap.soapObj.ldapws.wsdl, cfg.soap.soapObj.ldapws.wsdlOptions, (err, client) => {
            if (err) {
                reject({
                        code: error.code_03008,
                        desc: err
                    }) // failure
            } else {
                client.setEndpoint(cfg.soap.soapObj.ldapws.endpoint)
                if (cfg.soap.soapObj.ldapws.authenType) {
                    if (cfg.soap.soapObj.ldapws.authenType == 1) {
                        client.setSecurity(new soap.BasicAuthSecurity(cfg.soap.soapObj.ldapws.username, cfg.soap.soapObj.ldapws.password))
                    } else {
                        //No authen
                    }
                }
            ...
            parser.parseString(omResult, (err, dataJsonStr) => {
                if (err) {
                    ...
                    if (dataJsonStr.NewDataSet.Permission[0].MsgDetail[0] == 'Success' && util.isDataFound(dataJsonStr.NewDataSet.Table)) {
                        ...
                        reject({
                            code: error.code_03002,
                            desc: dataJsonStr
                        })
                    } else {
                        reject({
                            code: error.code_03006,
                            desc: dataJsonStr.NewDataSet.Permission[0].MsgDetail[0]
                        })
                    }
                }
            })
...
exports.omSearchOrgInfo = (req, params) => {
    return new Promise((resolve, reject) => {
        let jErr = {
            code: error.code_03001
        }
        try {
            this.soapCreateClient().then(client => {
                let args = {
                    OmCode: cfg.om.OmCode,
                    CompanyCode: params.companyCode,
                    OrgDescOrOrgCode: params.orgDesc
                }
                ...
                client.OM_WS_SearchOrgInfo(args, (err, result) => {
                    ...
                    if (err) {
                        ...
                        this.omParseString(result.OM_WS_SearchOrgInfoResult).then(table => {
                            ...
                            table.forEach(value => {
                                om.push({
                                    orgCode: value.ORGCODE[0],
                                    orgName: value.ORGNAME[0],
                                    orgDesc: value.ORGDESC[0],
                                    orgLevel: value.ORGLEVEL[0]
                                })
                            })
                        ...
                        this.omParseString(result.OM_WS_ListCompanyInWirelessResult).then(table => {
                            ...
                            table.forEach(value => {
                                om.push({
                                    ...
                                    companyCode: value.COMPANYCODE[0],
                                    descriptionEn: value.DESCRIPTION_EN[0],
                                    descriptionTh: value.DESCRIPTION_TH[0]
                                })
                            })
                            logger.debug(req, 'parserOmResult|' + util.jsonToText(om))
                            resolve(om)
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            this.soapCreateClient().then(client => {
                let args = {
                    OmCode: cfg.om.OmCode,
                    Username: util.getUserName(req.header('x-userTokenId'))
                }
                logger.debug(req, 'prepareOMrequest|' + util.jsonToText(args))
                    ...
                    async.doUntil(
                        (callback) => {
                            ...
                            cfg.om.approvalPosition.some(value => {
                                ...
                                if (om.managerPosition.indexOf(value) >= 0 && (value != 'VP' || (value == 'VP' && om.managerPosition.indexOf('AVP') < 0 && om.managerPosition.indexOf('SVP') < 0 && om.managerPosition.indexOf('EVP') < 0 && om.managerPosition.indexOf('SEVP') < 0))) r = true
                        ...
                        if (err) {
                            ...
                            this.omParseString(result.OM_WS_ListAllApproverResult).then(table => {
                                let om = {}
                                    ...
                                    // om.managerName=table[0].APPROVAL_THNAME[0]
                                    // om.managerSurname=table[0].APPROVAL_THSURNAME[0]
                                logger.debug(req, 'parserOmResult|' + util.jsonToText(table))
                                resolve(table)
                            }).catch(err => {
                                if (err.code) reject(err)
                                else {
...
exports.sendEmail = (to, cc, ur) => {
    return new Promise((resolve, reject) => {
        let jErr = {
            code: error.code_04001,
            desc: error.desc_04001
        }
        try {
...
exports.ldapAuthenticate = (ldapReq) => {
    return new Promise((resolve, reject) => {
        let jErr = {
            code: {},
            desc: {}
        ...
        try {
            this.createSoapClient().then(client => {
                let args = {
                    User: {
                    ...
                    ProjectCode: {
                        ProjectCode: ldapReq.projectCode,
                        ProjectLevel: ldapReq.projectLevel
                    }
                }
                logger.debug(null, '|LDAP Req: ' + util.jsonToText(args))
                client.Authenticate(args, (err, result) => {
                    logger.debug(null, '|LDAP Res' + util.jsonToText(result))
                    if (err) {
                        jErr.code = error.code_03009
                        jErr.desc = err
                        reject(jErr)
                    } else {
                        if (result.AuthenticateResult.Code == 0 && result.AuthenticateResult.Message == 'Success') {
                            resolve(result)
                        } else {
                            jErr.code = error.code_03010
                            jErr.desc = error.desc_03010
                            reject(jErr)
                        }
                    }
                }, {
                    timeout: cfg.soap.soapObj.ldapws.timeout
                })
            }).catch(err => {
                if (err.code) reject(err)
                else {
