...
function getLogHeader(logStatus,reqData) //${LogTime}|${hostname}|${appname}|INCOMING|Token|COMMAMD|Data|...
{
    ...
    let strHead = headTime+'|'+os.hostname()+logCfg.log.projectName
    
    if(reqData!=null && reqData!=''){
        // strHead = strHead + '|'+reqData.path+ '|IP:'+reqData.ip+'|'+reqData.originalUrl
        strHead = strHead+'|'+reqData.ssid+'|'+logStatus + '|'+reqData.method +'|API:'+reqData.url//+'|USER:' + util.getUserName(reqData.header('x-userTokenId'))
    }else strHead = strHead+'|'+logStatus
    return strHead
}
...
exports.db = (errMsg) =>
{
    writeLog(logCfg.log.logDbPath+'/'+getLogFileName(),getLogHeader('DB')+'|'+errMsg)
}
